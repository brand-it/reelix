...
/// Reorder TV episode files on FTP server based on episode swaps.
/// 
/// This function is used when episodes have been ripped out of order and need to be
/// renamed on the FTP server to match their correct episode numbers.
/// 
/// # Arguments
/// * `tv` - TV show metadata
/// * `season` - Season metadata  
/// * `swaps` - Vector of (from_episode, to_episode) tuples indicating which episodes to swap
/// * `state` - Application state containing FTP credentials
/// 
/// # Returns
/// * `Ok(usize)` - Number of files renamed on success
...
/// Connects to the FTP server using credentials from AppState.
/// 
/// This is a helper function used by episode swap functionality.
/// It connects, authenticates, but does NOT change directory.
